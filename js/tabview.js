(function() {
    if (OCA.Files && OCA.Files.Sidebar) {
        var MetadataTabView = function(el) {
            this.$el = $(el);

            this.update = function(fileInfo) {
                if (fileInfo) {
                    this.$el.html('<div style="text-align:center; word-wrap:break-word;" class="metadata-tab-view get-metadata"><p><img src="'
                    ...
                    var url = OC.generateUrl('/apps/metadata/get'),
                        data = {source: fileInfo.path + '/' + fileInfo.name},
                        _self = this;
                    $.ajax({
                        type: 'GET',
                        url: url,
        ...
        OCA.Files.Sidebar.registerTab(new OCA.Files.Sidebar.Tab({
            id: 'metadata',
            name: t('metadata', 'Metadata'),
            icon: 'icon-details',

            mount(el, fileInfo, context) {
                tabView = new MetadataTabView(el);
