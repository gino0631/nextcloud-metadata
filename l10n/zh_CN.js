OC.L10N.register(
    "metadata",
    {
    "Reading metadata …" : "正在读取元数据…",
    "Resolving, click here to view on map …" : "正在解决，请点击此处在地图上查看…",
    "Nominatim service unavailable, click here to view on map" : "名义上的服务不可用，请单击此处在地图上查看",
    "Location" : "地点",
    "Location could not be determined" : "无法确定位置",
    "Nominatim service unavailable" : "具体来说，服务不可用",
    "Metadata" : "元数据",
    "No metadata found." : "找不到元数据",
    "Unsupported MIME type \"%s\"." : "不支持的 MIME 类型 \"%s\"。",
    "EXIF support is missing; you might need to install an appropriate package for your system." : "EXIF 支持缺失；您可能需要为系统安装适当的软件包。",
    "Title" : "标题",
    "Artist" : "艺术家",
    "Date created" : "创建日期",
    "Date modified" : "修改日期",
    "Length" : "长度",
    "Dimensions" : "外型尺寸",
    "Frame rate" : "帧率",
    "%g fps" : "%g 帧",
    "Bit rate" : "比特率",
    "%s kbps" : "%s 千比特每秒",
    "Author" : "作者",
    "Copyright" : "版权",
    "Camera used" : "使用的相机",
    "Android version" : "安卓版本",
    "Video codec" : "视频编解码器",
    "Video sample size" : "视频样本大小",
    "%s bit" : "%s 比特",
    "Audio codec" : "音频编解码器",
    "Audio channels" : "音频频道",
    "Audio sample rate" : "音频采样率",
    "%s kHz" : "%s 千赫",
    "Audio sample size" : "音频样本大小",
    "Audio bit rate" : "音频采样率",
    "Album" : "专辑",
    "Track #" : "曲目编号 #",
    "Year" : "年份",
    "Date" : "日期",
    "Genre" : "曲风",
    "Comment" : "注释",
    "Encoded by" : "编码器",
    "Encoding tool" : "编码工具",
    "GPS coordinates" : "GPS 坐标",
    "GPS altitude" : "GPS 高度",
    "Headline" : "标题",
    "Description" : "描述",
    "Rating" : "评分",
    "Description writer" : "说明作者",
    "People" : "用户",
    "Tags" : "标签",
    "Keywords" : "关键词",
    "Instructions" : "说明",
    "Job title" : "职位名称",
    "Credits" : "致谢",
    "Source" : "来源",
    "Software" : "软件",
    "Exposure" : "曝光",
    "%s sec." : "%s 秒。",
    "ISO-%s" : "ISO-%s",
    "%s EV" : "%s EV",
    "Exposure program" : "曝光程序",
    "Exposure mode" : "曝光模式",
    "Focal length" : "焦距",
    "%g mm" : "%g 毫秒",
    "(35 mm equivalent: %g mm)" : "(35毫米等效焦段：%g 毫米)",
    "Max aperture" : "最大光圈",
    "Metering mode" : "测光模式",
    "Flash mode" : "闪光模式",
    "Subject" : "主题",
    "Created" : "已创建",
    "Modified" : "已修改",
    "Application" : "应用",
    "Number of pages" : "页数",
    "Trapped" : "陷印",
    "PDF producer" : "PDF 制作者",
    "PDF version" : "PDF 版本",
    "Number of files" : "文件数",
    "Not defined" : "未定义",
    "Manual" : "手动",
    "Normal program" : "标准程序",
    "Aperture priority" : "光圈优先",
    "Shutter priority" : "快门优先",
    "Creative program" : "创意程序",
    "Action program" : "动作程序",
    "Portrait mode" : "肖像模式",
    "Landscape mode" : "风景模式",
    "Auto exposure" : "自动曝光",
    "Manual exposure" : "手动曝光",
    "Auto bracket" : "自动支架",
    "Unknown" : "未知",
    "Average" : "平均测光",
    "Center Weighted Average" : "中央重点测光",
    "Spot" : "点测光",
    "Multi Spot" : "多点测光",
    "Pattern" : "图案",
    "Partial" : "局部测光",
    "Other" : "其他",
    "No flash function" : "没有闪光灯功能",
    "Flash" : "闪光灯",
    "auto" : "自动",
    "red-eye" : "去红眼",
    "strobe return" : "频闪回报",
    "H.264 - MPEG-4 AVC (part 10)" : "H.264-MPEG-4 AVC（第10部分）",
    "Yes" : "是",
    "No" : "否",
    "A plugin for displaying file metadata" : "用于显示文件元数据的插件",
    "A plugin which displays file metadata in the file details sidebar.\n\nSee the [README](https://github.com/gino0631/nextcloud-metadata/blob/master/README.md) for the supported file types and limitations." : "一个在文件详细信息侧栏中显示文件元数据的插件。\n\n有关受支持的文件类型和限制，请参见 [自述文件](https://github.com/gino0631/nextcloud-metadata/blob/master/README.md)。"
},
"nplurals=1; plural=0;");
